...
function authAdvertiser()
{
    return auth()->guard('advertiser')->user();
}

function authAdvertiserId()
{
...
function levelCommision($id, $amountSendByUser, $commissionType, $trx)
{
    ...
    while ($usr != "" || $usr != "0" || $i < $level) {
        $me = Advertiser::find($usr);
        $refer = Advertiser::find($me->ref_by);
        if ($refer == "") {
            break;
        }
        ...
        $referWallet = Advertiser::where('id', $refer->id)->first();
        ...
        $transaction->advertiser_id  = $refer->id;

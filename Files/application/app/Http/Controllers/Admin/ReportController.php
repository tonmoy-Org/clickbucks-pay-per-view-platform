...
class ReportController extends Controller
{
    public function transaction(Request $request)
    {
        ...
        $transactions = Transaction::with('user')->where('user_id','!=',0)->orderBy('id','desc');
        if ($request->search) {
    ...
    public function notificationHistory(Request $request){
        ...
        $logs = $logs->with('user')->where('advertiser_id','=',null)->paginate(getPaginate());
    ...
    public function emailDetails($id){
     ...
     {
         $pageTitle = 'Avdertiser Transaction Logs';
        ...
        //  date search
         ...
         $transactions = $transactions->paginate(getPaginate());
         return view('admin.advertisers.reports.transactions', compact('pageTitle', 'transactions','remarks'));
     }

     public function advertiserLoginHistory(Request $request)
     {
         $loginLogs = UserLogin::orderBy('id','desc')->with('advertiser')->where('advertiser_id','!=',0);
         $pageTitle = 'Avdertiser Login History';
         if ($request->search) {
             $search = $request->search;
             $pageTitle = 'Avdertiser Login History - ' . $search;
             $loginLogs = $loginLogs->whereHas('advertiser', function ($query) use ($search) {
                 $query->where('username', $search);
             });
         }
         $loginLogs = $loginLogs->paginate(getPaginate());
         return view('admin.advertisers.reports.logins', compact('pageTitle', 'loginLogs'));
     }

     public function advertiserLoginIpHistory($ip)
     {
         $pageTitle = 'Login by - ' . $ip;
         $loginLogs = UserLogin::where('user_ip',$ip)->orderBy('id','desc')->with('avdertiser')->paginate(getPaginate());
         return view('admin.advertisers.reports.logins', compact('pageTitle', 'loginLogs','ip'));
     ...
     public function advertiserNotificationHistory(Request $request){
         ...
         $search = $request->search;
         if($search) {
             $logs = $logs->whereHas('avdertiser', function ($user) use ($search) {
                 $user->where('username', 'like',"%$search%");
             });

         }
         $logs = $logs->with('advertiser')->whereNotNull('advertiser_id')->paginate(getPaginate());
         return view('admin.advertisers.reports.notification_history', compact('pageTitle','logs'));
     }

     public function advertiserEmailDetails($id){
         $pageTitle = 'Email Details';
         $email = NotificationLog::findOrFail($id);
         return view('admin.avdertisers.reports.email_details', compact('pageTitle','email'));
     }
